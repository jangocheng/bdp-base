...
case class FfqDataProcessService () {

  def persistFfqOrderData(resultArray: Array[(String, Float)]): Unit = {
  ...
  def persistFfqOrderCityData(resultArray: Array[(String, Int)]): Unit = {
    resultArray.foreach(param => {
      RedisOperationX.hmIncrementByIntOrCreate(ApplicationCommon.FFQ_CITY_RANK, param._1, param._2)
    })
  }
}
