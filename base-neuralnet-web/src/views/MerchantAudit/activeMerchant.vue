...
import { getActiveMechantData } from '@/api/business/merchantAudit.js';
...
export default {
  ...
  data() {
    return {
      ...
      xData: [],
      newAmount: [], //新增数
      firstTrialAmount: [], //初审数
      accessApplyAmount: [], //准入申请数
      approvalRefuseAmount: [], //审批拒绝数
      closeAmount: [], //关闭数
      activationAmount: [], //激活数
    };
  },
  computed: {
    legendArr() {
      return [
        ...
        },
        {
          name: '关闭数',
          type: this.isChain ? 'line' : 'bar',
          data: this.closeAmount,
        },
        {
          name: '激活数',
          type: this.isChain ? 'line' : 'bar',
          data: this.activationAmount,
        },
      ];
    },
    sidebarStatus() {
    ...
    text() {
      const INSTANT_STR = '通过与激活门店趋势';
      const CHAIN_STR = '通过与激活门店趋势"环比"';
  ...
  methods: {
    ...
    async getActiveMechantData() {
      const _init = () => {
        this.xData = [];
        this.newAmount = [];
        this.firstTrialAmount = [];
        this.accessApplyAmount = [];
        this.approvalRefuseAmount = [];
        this.closeAmount = [];
      ...
      try {
        ...
        const res = await getActiveMechantData(params);
        if (!res || !res.data) {
          return;
        }
        ...
        temResArr.forEach(ele => {
          ...
          this.newAmount.push(ele.newAmount);
          this.firstTrialAmount.push(ele.firstTrialAmount);
          this.accessApplyAmount.push(ele.accessApplyAmount);
          this.approvalRefuseAmount.push(ele.approvalRefuseAmount);
          this.closeAmount.push(ele.closeAmount);
          this.activationAmount.push(ele.activationAmount);
        });
        this.updateEchartsOption(this.option);
      } catch (error) {
