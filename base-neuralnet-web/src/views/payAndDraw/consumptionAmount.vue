...
import { getPayAmount } from '@/api/payAndDraw.js';
      ...
      for (const key in this.echartsData) {
        if (this.echartsData.hasOwnProperty(key)) {
          ...
          this.echartsData[key].forEach(ele => {
            ...
            temLineObj.data.push(ele.amount);
          });
  ...
  methods: {
    ...
    async getPayAmount() {
      ...
      try {
        ...
        _init();
        const res = await getPayAmount(params);
        if (!res || !res.data) {
          return;
        }
