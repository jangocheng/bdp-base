...
import { getPayMaximum } from '@/api/payAndDraw.js';
    ...
    option() {
      ...
      for (const key in this.echartsData) {
        if (this.echartsData.hasOwnProperty(key)) {
          ...
          this.echartsData[key].forEach(ele => {
            temLineObj.name = ele.consumptionType;
            temLineObj.data.push(ele.sum);
          });
  ...
  methods: {
    ...
    async getPayMaximum() {
      ...
      try {
        ...
        _init();
        const res = await getPayMaximum(params);
        if (!res || !res.data) {
          return;
        }
