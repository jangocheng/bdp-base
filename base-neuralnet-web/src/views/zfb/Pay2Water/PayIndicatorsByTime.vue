...
import { getPayIndicatorsByTimeQuery } from '@/api/zfb/getPay2Water.js';
...
export default {
  ...
  data() {
    return {
      id: 'PayIndicatorsByTime',
      chart: '',
      // false即时状态 | true环比状态
      isChain: false,
      // 数据
      xData: [],
      staffPayCnt: [], //内部员工支付次数
      socialPayCnt: [], //社会人士支付次数
      payCnt: [], //支付次数合计
      payAvg: [], //平均支付金额
      payMax: [], //最高支付金额
      staffPaySum: [], //内部员工支付金额
      socialPaySum: [], //社会人士支付金额
      paySum: [], //支付金额合计
      payDiscountCnt: [], //折扣笔数
      payDiscountSum: [], //折扣金额
    };
      ...
      return [
        {
          name: '内部员工支付次数',
          type: 'line',
          data: this.staffPayCnt,
        },
        {
          name: '社会人士支付次数',
          type: 'line',
          data: this.socialPayCnt,
        },
        {
          name: '支付次数合计',
          type: 'line',
      ...
      return [
        {
          ...
          data: this.staffPaySum,
        },
        {
          name: '社会员工支付金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.socialPaySum,
        },
        {
          name: '支付金额合计',
          type: this.isChain ? 'line' : 'bar',
          ...
          feature: {
            myTool2: {
              ...
              onclick: () => {
                // this.getCurrData();
                this.isChain = false;
                this.getPayIndicatorsByTimeQuery();
              },
            },
            myTool1: {
              show: true,
  ...
  methods: {
    ...
    async getPayIndicatorsByTimeQuery() {
      const _init = () => {
        this.xData = [];
        this.staffPayCnt = []; //内部员工支付次数
        this.socialPayCnt = []; //社会人士支付次数
        this.payCnt = []; //支付次数合计
        this.payAvg = []; //平均支付金额
        this.payMax = []; //支付次数合计
        this.staffPaySum = []; //内部员工支付金额
        this.socialPaySum = []; //社会人士支付金额
        this.paySum = []; //支付金额合计
        this.payDiscountCnt = []; //折扣笔数
        this.payDiscountSum = []; //折扣金额
      };
      try {
        this.loading = true;
        const params = {
        ...
        const res = await getPayIndicatorsByTimeQuery(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.staffPayCnt.push(ele.staffPayCnt);
            this.socialPayCnt.push(ele.socialPayCnt);
            this.payCnt.push(ele.payCnt);
            this.payAvg.push(ele.payAvg);
            this.payMax.push(ele.payMax);
            this.staffPaySum.push(ele.staffPaySum);
            this.socialPaySum.push(ele.socialPaySum);
            this.paySum.push(ele.paySum);
            this.payDiscountCnt.push(ele.payDiscountCnt);
            this.payDiscountSum.push(ele.payDiscountSum);
          });
        }
        this.loading = false;
        this.updateEchartsOption(this.option);
