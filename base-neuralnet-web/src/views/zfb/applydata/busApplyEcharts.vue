...
import { getConditionQuery } from '@/api/zfb/getApplyData';
    ...
    legendArr() {
      return [
        ...
        },
        {
          name: '拒绝率',
          type: 'line',
          data: this.refuseRate,
        },
        {
          name: '取消率',
          type: 'line',
          data: this.cancelRate,
        },
      ];
    },
    sidebarStatus() {
  ...
  methods: {
    ...
    async getConditionQuery() {
      ...
      try {
        ...
        const res = await getConditionQuery(params);
        _init();
        this.loading = false;
        res.data.forEach(ele => {
          this.xData.push(ele.bizDate);
          this.applySum.push(ele.applySum);
          this.applyRate.push(ele.applyRate);
          this.passRate.push(ele.passRate);
          this.refuseRate.push(ele.refuseRate);
          this.cancelRate.push(ele.cancelRate);
        });
        this.updateEchartsOption(this.option);
      } catch (error) {
