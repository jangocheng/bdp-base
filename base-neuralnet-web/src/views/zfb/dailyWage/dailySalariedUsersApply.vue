...
import { creditQuotaApplyQuery } from '@/api/zfb/getDailyWage.js';
...
export default {
  ...
  data() {
    return {
      ...
      xData: [],
      quotaCnt: [], //薪资额度激活人数
      quotaCntRatio: [], //薪资额度激活占比
      cnt: [], //信用额度申请人数
      quotaApplyCntRatio: [], //信用额度申请占比
      newZfbQuotaApplyCnt: [], //新增信用额度申请数
      newZfbQuotaPassCnt: [], //新增信用额度申请通过数
      newZfbQuotaPassRate: [], //新增信用额度申请通过率
    };
  },
  computed: {
    legendArr() {
      ...
      return [
        {
          name: '新增信用额度申请数',
          type: this.isChain ? 'line' : 'bar',
          data: this.newZfbQuotaApplyCnt,
        },
        {
          name: '新增信用额度申请通过数',
          type: this.isChain ? 'line' : 'bar',
          data: this.newZfbQuotaPassCnt,
        },
        {
          name: '新增信用额度申请通过率',
          type: 'line',
          data: this.newZfbQuotaPassRate,
        },
      ];
    },
    sidebarStatus() {
  ...
  methods: {
    ...
    async creditQuotaApplyQuery() {
      const _init = () => {
        this.xData = [];
        this.quotaCnt = [];
        this.quotaCntRatio = [];
        this.cnt = [];
        this.quotaApplyCntRatio = [];
        this.newZfbQuotaApplyCnt = [];
        this.newZfbQuotaPassCnt = [];
        this.newZfbQuotaPassRate = [];
      };
      try {
        this.loading = true;
        const params = {
        ...
        const res = await creditQuotaApplyQuery(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.quotaCnt.push(ele.quotaCnt);
            this.quotaCntRatio.push(ele.quotaCntRatio);
            this.cnt.push(ele.cnt);
            this.quotaApplyCntRatio.push(ele.quotaApplyCntRatio);
            this.newZfbQuotaApplyCnt.push(ele.newZfbQuotaApplyCnt);
            this.newZfbQuotaPassCnt.push(ele.newZfbQuotaPassCnt);
            this.newZfbQuotaPassRate.push(ele.newZfbQuotaPassRate);
          });
        }
        this.loading = false;
        this.updateEchartsOption(this.option);
