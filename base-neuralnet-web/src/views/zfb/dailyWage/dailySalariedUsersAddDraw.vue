...
import { quotaNewWithdrawQuery } from '@/api/zfb/getDailyWage.js';
...
export default {
  ...
  data() {
    return {
      id: 'dailySalariedUsersAddDraw',
      chart: '', // false即时状态 | true环比状态
      isChain: false,
      /**
       * Echarts数据
       */
      xData: [],
      newZfbQuotaWithdrawSum: [], //新增提现金额
      newZfbQuotaWithdrawCntPv: [], //新增提现笔数
      newZfbQuotaWithdrawSumAvg: [], //新增信用额度笔均提现金额
      newZfbQuotaWithdrawSumRatio: [], //提现金额占比
      newZfbQuotaWithdrawCntPvRatio: [], //提现笔数占比
    };
  },
  computed: {
    legendArr() {
      return [
        {
          ...
          data: this.newZfbQuotaWithdrawSum,
        },
        {
          name: '新增提现笔数',
          type: this.isChain ? 'line' : 'bar',
          data: this.newZfbQuotaWithdrawCntPv,
        },
        {
          name: '新增提现笔均金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.newZfbQuotaWithdrawSumAvg,
        },
        {
          name: '新增提现金额占比',
          type: 'line',
          data: this.newZfbQuotaWithdrawSumRatio,
        },
        {
          name: '新增提现笔数占比',
          type: 'line',
          data: this.newZfbQuotaWithdrawCntPvRatio,
        },
      ];
    },
    sidebarStatus() {
    ...
    text() {
      const INSTANT_STR = '日结薪用户新增信用额度提现情况';
  ...
  methods: {
    ...
    async quotaNewWithdrawQuery() {
      const _init = () => {
        this.xData = [];
        this.newZfbQuotaWithdrawSum = [];
        this.newZfbQuotaWithdrawCntPv = [];
        this.newZfbQuotaWithdrawSumAvg = [];
        this.newZfbQuotaWithdrawSumRatio = [];
        this.newZfbQuotaWithdrawCntPvRatio = [];
      };
      try {
        this.loading = true;
        const params = {
        ...
        const res = await quotaNewWithdrawQuery(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.newZfbQuotaWithdrawSum.push(ele.newZfbQuotaWithdrawSum);
            this.newZfbQuotaWithdrawCntPv.push(ele.newZfbQuotaWithdrawCntPv);
            this.newZfbQuotaWithdrawSumAvg.push(ele.newZfbQuotaWithdrawSumAvg);
            this.newZfbQuotaWithdrawSumRatio.push(
              ele.newZfbQuotaWithdrawSumRatio,
            );
            this.newZfbQuotaWithdrawCntPvRatio.push(
              ele.newZfbQuotaWithdrawCntPvRatio,
            );
          });
        }
        this.loading = false;
