...
import { quotaPayQuery } from '@/api/zfb/getDailyWage.js';
...
export default {
  ...
  data() {
    return {
       ...
       */
      xData: [],
      quotaShopPaySum: [], //商城支付金额
      quotaNotShopPaySum: [], //非商城支付金额
      quotaPaySum: [], //总支付金额
      quotaPayCntUv: [], //支付人数
      quotaPayCntPv: [], //支付笔数
      quotaPaySumAvg: [], //笔均金额
      ...
      return [
        {
          ...
          data: this.quotaShopPaySum,
        },
        {
          name: '非商城支付金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.quotaNotShopPaySum,
        },
        {
          name: '总支付金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.quotaPaySum,
        },
        {
          name: '支付人数',
          type: 'line',
          data: this.quotaPayCntUv,
        },
        {
          name: '支付笔数',
          type: 'line',
          data: this.quotaPayCntPv,
        },
        {
          name: '笔均金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.quotaPaySumAvg,
        },
      ];
    },
    sidebarStatus() {
  ...
  methods: {
    ...
    async quotaPayQuery() {
      const _init = () => {
        this.xData = [];
        this.quotaShopPaySum = [];
        this.quotaNotShopPaySum = [];
        this.quotaPaySum = [];
        this.quotaPayCntUv = [];
        this.quotaPayCntPv = [];
        this.quotaPaySumAvg = [];
      ...
      try {
        ...
        const res = await quotaPayQuery(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.quotaShopPaySum.push(ele.quotaShopPaySum);
            this.quotaNotShopPaySum.push(ele.quotaNotShopPaySum);
            this.quotaPaySum.push(ele.quotaPaySum);
            this.quotaPayCntUv.push(ele.quotaPayCntUv);
            this.quotaPayCntPv.push(ele.quotaPayCntPv);
            this.quotaPaySumAvg.push(ele.quotaPaySumAvg);
          });
        }
        this.loading = false;
