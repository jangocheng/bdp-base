...
import { quotaWithdrawQuery } from '@/api/zfb/getDailyWage.js';
...
export default {
  ...
  data() {
    return {
       ...
       */
      xData: [],
      salaryWithdrawSum: [], //薪资额度总提现金额
      salaryWithdrawCntPv: [], //薪资额度总提现笔数
      salaryWithdrawSumAvg: [], //薪资额度提现笔均金额
      withdrawSum: [], //信用额度总提现金额
      withdrawCntPv: [], //信用额度总提现笔数
      withdrawSumAvg: [], //信用额度提现笔均金额
    };
  },
  computed: {
    legendArr() {
      return [
        {
          ...
          data: this.salaryWithdrawSum,
        },
        {
          name: '薪资额度总提现笔数',
          type: this.isChain ? 'line' : 'bar',
          data: this.salaryWithdrawCntPv,
        },
        {
          name: '薪资额度提现笔均金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.salaryWithdrawSumAvg,
        },
        {
          name: '信用额度总提现金额',
          type: this.isChain ? 'line' : 'bar',
        ...
        },
        {
          name: '信用额度总提现笔数',
          type: this.isChain ? 'line' : 'bar',
          data: this.withdrawCntPv,
        },
        {
          name: '信用额度提现笔均金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.withdrawSumAvg,
        },
      ];
    },
    ...
    text() {
      const INSTANT_STR = '日结薪用户薪资额度和信用额度提现情况';
  ...
  methods: {
    ...
    async quotaWithdrawQuery() {
      const _init = () => {
        this.xData = [];
        this.salaryWithdrawSum = [];
        this.salaryWithdrawCntPv = [];
        this.salaryWithdrawSumAvg = [];
        this.withdrawSum = [];
        this.withdrawCntPv = [];
        this.withdrawSumAvg = [];
      };
      try {
        ...
        const res = await quotaWithdrawQuery(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.salaryWithdrawSum.push(ele.salaryWithdrawSum);
            this.salaryWithdrawCntPv.push(ele.salaryWithdrawCntPv);
            this.salaryWithdrawSumAvg.push(ele.salaryWithdrawSumAvg);
            this.withdrawSum.push(ele.withdrawSum);
            this.withdrawCntPv.push(ele.withdrawCntPv);
            this.withdrawSumAvg.push(ele.withdrawSumAvg);
          });
        }
        this.loading = false;
