...
import { getCreditAmountQuery } from '@/api/zfb/getPay2WithdrawalsData';
...
export default {
  ...
  data() {
    return {
      ...
      xData: [],
      passSum: [], //整体授信金额
      staffPassSum: [], //内部员工授信金额
      staffPassRate: [], // 内部员工授信金额占比
      socialPassSum: [], //社会人士授信金额
      socialPassRate: [], //社会人士授信金额占比
    };
  },
  computed: {
    legendArr() {
      return [
        ...
        },
        {
          name: '内部员工授信金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.staffPassSum,
        },
        {
          name: '社会人士授信金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.socialPassSum,
        },
        {
          name: '内部员工授信占比',
          type: 'line',
          data: this.staffPassRate,
        },
        {
          name: '社会员工授信占比',
          type: 'line',
          data: this.socialPassRate,
        },
      ];
    },
    sidebarStatus() {
  ...
  methods: {
    ...
    async getCreditAmountQuery() {
      ...
      try {
        ...
        const res = await getCreditAmountQuery(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.passSum.push(ele.passSum);
            this.staffPassSum.push(ele.staffPassSum);
            this.staffPassRate.push(ele.staffPassRate);
            this.socialPassSum.push(ele.socialPassSum);
            this.socialPassRate.push(ele.socialPassRate);
          });
        }
        this.loading = false;
        this.updateEchartsOption(this.option);
