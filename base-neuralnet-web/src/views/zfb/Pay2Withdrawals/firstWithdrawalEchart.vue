...
import { firstWithdrawal } from '@/api/zfb/getPay2WithdrawalsData';
...
export default {
  ...
  data() {
    return {
      id: 'firstWithdrawalEchart',
      chart: '',
      isChain: false,
      // 数据
      xData: [],
      firstWithdrawCnt: [], //首次提现次数
      staffFirstWithdrawSum: [], //内部员工首次提现金额
      staffFirstWithdrawCnt: [], //内部员工首次提现次数
      firstWithdrawCntRate: [], //首次提现总次数占比
      staffFirstWithdrawSumRate: [], //内部员工首次提现金额占比
      staffFirstWithdrawCntRate: [], //内部员工首次提现次数占比
      firstWithdrawSum: [], //首次提现总金额
      socialFirstWithdrawSum: [], //社会人士首次提现金额
      socialFirstWithdrawCnt: [], //社会人士首次提现次数
      firstWithdrawSumRate: [], //首次提现总金额占比
      socialFirstWithdrawSumRate: [], //社会人士首次提现金额占比
      socialFirstWithdrawCntRate: [], //社会人士首次提现次数占比
    };
  },
  computed: {
    legendArr() {
      return [
        {
          ...
          data: this.firstWithdrawSum,
        },
        {
          name: '首次提现总金额占比',
          type: 'line',
          data: this.firstWithdrawSumRate,
        },
        {
          name: '内部员工首次提现金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.staffFirstWithdrawSum,
        },
        {
          name: '社会人士首次提现金额',
          type: this.isChain ? 'line' : 'bar',
          data: this.socialFirstWithdrawSum,
        },
        {
          name: '内部员工首次提现金额占比',
          type: 'line',
          data: this.staffFirstWithdrawSumRate,
        },
        {
          name: '社会人士首次提现金额占比',
          type: 'line',
          data: this.socialFirstWithdrawSumRate,
        },
      ];
    },
    legendArr2() {
      return [
        {
          name: '首次提现总次数',
          type: this.isChain ? 'line' : 'bar',
          data: this.firstWithdrawCnt,
        },
        {
          name: '首次提现总次数占比',
          type: 'line',
          data: this.firstWithdrawCntRate,
        },
        {
          name: '内部员工首次提现次数',
          type: this.isChain ? 'line' : 'bar',
          data: this.staffFirstWithdrawCnt,
        },
        {
          name: '社会人士首次提现次数',
          type: this.isChain ? 'line' : 'bar',
          data: this.socialFirstWithdrawCnt,
        },
        {
          name: '内部员工首次提现次数占比',
          type: 'line',
          data: this.staffFirstWithdrawCntRate,
        },
        {
          name: '社会人士首次提现次数占比',
          type: 'line',
          data: this.socialFirstWithdrawCntRate,
        },
      ];
    },
    sidebarStatus() {
    ...
    text() {
      const INSTANT_STR = '首次提现次数、金额及占比';
  ...
  methods: {
    ...
    async firstWithdrawal() {
      const _init = () => {
        this.xData = [];
        this.firstWithdrawCnt = [];
        this.staffFirstWithdrawSum = [];
        this.staffFirstWithdrawCnt = [];
        this.firstWithdrawCntRate = [];
        this.staffFirstWithdrawSumRate = [];
        this.staffFirstWithdrawCntRate = [];
        this.firstWithdrawSum = [];
        this.socialFirstWithdrawSum = [];
        this.socialFirstWithdrawCnt = [];
        this.firstWithdrawSumRate = [];
        this.socialFirstWithdrawSumRate = [];
        this.socialFirstWithdrawCntRate = [];
      };
      try {
        this.loading = true;
        const params = {
        ...
        const res = await firstWithdrawal(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.firstWithdrawCnt.push(ele.firstWithdrawCnt);
            this.staffFirstWithdrawSum.push(ele.staffFirstWithdrawSum);
            this.staffFirstWithdrawCnt.push(ele.staffFirstWithdrawCnt);
            this.firstWithdrawCntRate.push(ele.firstWithdrawCntRate);
            this.staffFirstWithdrawSumRate.push(ele.staffFirstWithdrawSumRate);
            this.staffFirstWithdrawCntRate.push(ele.staffFirstWithdrawCntRate);
            this.firstWithdrawSum.push(ele.firstWithdrawSum);
            this.socialFirstWithdrawSum.push(ele.socialFirstWithdrawSum);
            this.socialFirstWithdrawCnt.push(ele.socialFirstWithdrawCnt);
            this.firstWithdrawSumRate.push(ele.firstWithdrawSumRate);
            this.socialFirstWithdrawSumRate.push(
              ele.socialFirstWithdrawSumRate,
            );
            this.socialFirstWithdrawCntRate.push(
              ele.socialFirstWithdrawCntRate,
            );
          });
        }
        this.loading = false;
