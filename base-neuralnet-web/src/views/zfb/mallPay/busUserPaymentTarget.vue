...
import { getUserPayTarget } from '@/api/zfb/mallPay.js';
...
export default {
      ...
      return [
        {
          name: '新用户数',
          type: 'line',
          data: this.newPayUserCnt,
        },
        {
          name: '支付用户数',
          type: 'line',
          data: this.payUserCnt,
        },
        {
          name: '支付件数',
          type: 'line',
          data: this.payOrderCnt,
        },
        {
          name: '支付金额',
  ...
  methods: {
    ...
    async getUserPayTarget() {
      const _init = () => {
        this.xData = [];
        this.payUserCnt = []; //支付用户数
        this.newPayUserCnt = []; //新用户数
        this.payOrderCnt = []; //支付件数
        this.paySum = []; //支付金额
        this.payAvg = []; //均件金额
      };
      try {
        ...
        const res = await getUserPayTarget(params);
        _init();
        if (res.data && res.data.length > 0) {
          res.data.forEach(ele => {
            this.xData.push(ele.bizDate);
            this.payUserCnt.push(ele.payUserCnt);
            this.newPayUserCnt.push(ele.newPayUserCnt);
            this.payOrderCnt.push(ele.payOrderCnt);
