<template>
  <div class="main">
    <!-- <e-graph id="busEcharts" :option="option" class="echart-block"></e-graph> -->
...
import { getRegisterData } from '@/api/business/storeAudit.js';
...
export default {
  ...
  data() {
    return {
      ...
      xData: [],
      applyAmount: [], //申请数
      accessFirstTrialAmount: [], //实名认证数
      accessApplyAmount: [], //实名认证率
      approvalRefuseAmount: [], //申请件数
      applyApprovedAmount: [], //申请通过率
    };
  },
  computed: {
    legendArr() {
      return [
        {
          ...
          data: this.approvalRefuseAmount,
        },
        {
          name: '初审数',
          type: this.isChain ? 'line' : 'bar',
          data: this.accessFirstTrialAmount,
        },
        {
          name: '准入申请数',
          type: this.isChain ? 'line' : 'bar',
          data: this.accessApplyAmount,
        },
        {
          name: '审批拒绝数',
          type: this.isChain ? 'line' : 'bar',
  ...
  methods: {
    ...
    init() {
      ...
      if (this.option) this.updateEchartsOption(this.option);
    },
    // 请求注册数据
    async getRegisterData() {
      const _init = () => {
        this.xData = [];
        this.applyAmount = []; //申请数
        this.accessFirstTrialAmount = []; //初审数
        this.accessApplyAmount = []; //准入申请
        this.approvalRefuseAmount = []; //审批拒绝
        this.applyApprovedAmount = []; //审批通过
      };
      try {
        const params = {
        ...
        const res = await getRegisterData(params);
        if (!res || !res.data) return;
        const temResArr = res.data;
        _init();
        temResArr.forEach(ele => {
          this.xData.push(ele.date);
          this.applyAmount.push(ele.applyAmount);
          this.accessFirstTrialAmount.push(ele.accessFirstTrialAmount);
          this.accessApplyAmount.push(ele.accessApplyAmount);
          this.approvalRefuseAmount.push(ele.approvalRefuseAmount);
          this.applyApprovedAmount.push(ele.applyApprovedAmount);
        });
