...
import { getStoreAuditApplyTableData } from '@/api/business/storeAudit.js';
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      this.pageNum = val;
      this.getStoreAuditApplyTableData();
    },
    /**
     * 获取渠道申请报表
     * @param status 是否是父组件调用，是传入true
     */
    async getStoreAuditApplyTableData(status = false) {
      if (status) {
        this.pageNum = 1;
      }
      // 生成参数对象
      ...
      // 格式化对象
      const res = await getStoreAuditApplyTableData(param);
      if (!res || !res.data) {
        return;
      }
