...
import { getChannelApplyData } from '@/api/business/getBusApplyData.js';
...
export default {
  ...
  computed: {
    ...
    option() {
      ...
      return {
        ...
        // color: ['#61A0A7', '#D48266', '#C23531', '#91C7AF', '#2F4554'],
        calculable: true,
        xAxis: [
          {
            type: 'category',
    ...
    text() {
      const INSTANT_STR = '渠道申请量占比';
      const CHAIN_STR = '渠道申请量占比及"环比"';
  ...
  methods: {
    ...
    /**
     * 从小到大排序数组，，根据某个key比较对象
     */
    dateBubbleSort2(arr, key) {
    ...
    async getChannelApplyData() {
      ...
      try {
        ...
        const res = await getChannelApplyData(params);
        const temResArr = res.data;
        if (!temResArr) {
          return;
        }
        _init();
        const { channelData, objByChannel } = temResArr;
        let temArr = [];
        {
          // 将结构平铺成一维数组
          for (const key in objByChannel) {
            if (objByChannel.hasOwnProperty(key)) {
              const ele = objByChannel[key];
              temArr = [...ele, ...temArr];
            }
          }
          // 获取时间，进行排序，小在前，大在后
          const temDateSortArr = temArr.map(ele => ele.date);
          // 遍历时间，根据对应的时间组成渠道对象,得到X轴数据
          this.xData = [...new Set(temDateSortArr)];
          this.xData.sort((x, y) => {
            if (x < y) return -1;
            if (x > y) return 1;
            return 0;
        ...
        // 将每个渠道对象下的数据时间拿出来与temDateSortArr相比，补全缺口

        for (const key in objByChannel) {
          if (objByChannel.hasOwnProperty(key)) {
            // 先获取当前渠道下的时间list
            const temDateList = objByChannel[key].map(item => item.date);
            // 获取差集
            const minux = this.xData.filter(x => temDateList.indexOf(x) === -1);
            // 遍历差集，将其缺口时间填充进去
            minux.map(item => {
              objByChannel[key].push({
                date: item,
                channel: key,
                registerApproved: 0,
              });
        ...
        this.legendArr = channelData.map(item => {
          // 拿出当前渠道的数组
          const ChannelArr = objByChannel[item];
          const resArr = ChannelArr.map(ele => ele.registerApproved);
          return {
            name: item,
            data: resArr,
