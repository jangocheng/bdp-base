...
import { numFormatter, objClone } from '@/utils/index';
import { getApplyEchartData } from '@/api/business/getBusApplyData.js';
...
export default {
  ...
  computed: {
    legendArr() {
      return [
        ...
        },
        {
          name: '申请通过率',
          type: 'line',
          data: this.applyApproved,
        },
        {
          name: '审批中件数',
          type: 'bar',
          data: this.applyApprovingAmount,
        },
        {
          name: '获批额度之和',
          type: this.isChain ? 'line' : 'bar',
          data: this.approvedCreditSum,
        },
      ];
    },
    sidebarStatus() {
    ...
    text() {
      const INSTANT_STR = '申请环节数量';
      const CHAIN_STR = '申请环节"环比"';
  ...
  methods: {
    ...
    async getApplyEchartData() {
      const _init = () => {
        this.xData = [];
        this.applyAmount = [];
        this.applyApprovedAmount = [];
        this.applyApproved = [];
        this.approvedCreditSum = [];
        this.applyApprovingAmount = [];
      };
      try {
        const params = {
        ...
        const res = await getApplyEchartData(params);
        ...
        temResArr.forEach(ele => {
          ...
          this.applyApproved.push(ele.applyApproved);
          this.applyApprovingAmount.push(ele.applyApprovingAmount);
          this.approvedCreditSum.push(ele.approvedCreditSum);
        });
        this.updateEchartsOption(this.option);
      } catch (error) {
