...
<template>
  <div class="app-container">
    <el-card class="header-card" shadow="hover">
      <div class="header-card-content">
        <div class="header-card-selection">
          ...
          <div>
            ...
            >
              <el-option
                v-for="(item, index) in channelEunm"
                :key="index"
                :label="item"
                :value="item"
              ></el-option>
          ...
          <div>
            ...
            >
              <el-option
                v-for="(item, index) in osTypeEunm"
                :key="index"
                :label="item.label"
                :value="item.value"
              ></el-option>
      ...
      <div class="header-card-content">
        <div class="header-card-selection" style="margin-top:10px">
          <div>
            <span class="header-card-title">客户类型:</span>
            <!-- class="select-smallwidth" -->
            <el-select v-model="customer" placeholder="请选择">
              <el-option
                v-for="(item, index) in customerEunm"
                :key="index"
                :label="item.label"
                :value="item.value"
              ></el-option>
          ...
          <div>
            <span class="header-card-title">审核类型:</span>
            <el-select
              v-model="checkType"
              placeholder="请选择"
              class="select-smallwidth"
            >
              <el-option
                v-for="(item, index) in checkTypeEunm"
                :key="index"
                :label="item.label"
                :value="item.value"
              ></el-option>
...
<script>
import { getApplyChanel } from '@/api/business/getBusApplyData.js';
import {
  checkTypeEunm,
  customerEunm,
  osTypeEunm,
...
import busChanelApplyEcharts from './busChanelApplyEcharts.vue';
...
export default {
  ...
  methods: {
    ...
    healderQuery() {
      ...
      // 更新渠道报表
      this.$refs.BusApplyTable.getBusApplyTableData(true);
      //更新“申请环节进件”echart数据
      this.$refs.BusApplyEcharts.getApplyEchartData();
      this.$refs.BusApplyChanel.getChannelApplyData();
    },
    // 获取渠道数据
    async getApplyChanel() {
      // 发起渠道请求
      const res = await getApplyChanel();
      this.channelEunm = res.data;
      this.healderQuery();
    },
    // 初始化时间
    initTimeSeries() {
