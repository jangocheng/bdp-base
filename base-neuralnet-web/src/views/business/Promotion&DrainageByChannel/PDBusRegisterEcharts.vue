...
import { getPDBusRegisterData } from '@/api/business/PDChannel.js';
...
export default {
  ...
  computed: {
      ...
      return [
        {
          ...
          data: this.registerAmount,
        },
        {
          name: '实名认证数',
          type: this.isChain ? 'line' : 'bar',
          data: this.identifiedAmount,
        },
        {
          name: '申请数',
          type: 'bar',
          data: this.applyAmount,
        },
        {
          name: '申请通过数',
          type: 'bar',
        ...
        },
        {
          name: '机审拒绝人数',
          type: 'bar',
          data: this.machineRefuseAmount,
        },
        {
          name: '提现人数',
          type: this.isChain ? 'line' : 'bar',
          data: this.withdrawPassAmount,
        },
      ];
    },
    sidebarStatus() {
    ...
    text() {
      const INSTANT_STR = '渠道注册数量';
      const CHAIN_STR = '渠道注册"环比"';
  ...
  methods: {
    ...
    async getPDBusRegisterData() {
      const _init = () => {
        this.xData = [];
        this.registerAmount = [];
        this.identifiedAmount = [];
        this.applyAmount = [];
        this.applyApprovedAmount = [];
        this.machineRefuseAmount = [];
        this.withdrawPassAmount = [];
      };
      try {
        const params = {
        ...
        const res = await getPDBusRegisterData(params);
        ...
        temResArr.forEach(ele => {
          ...
          this.registerAmount.push(ele.registerAmount);
          this.identifiedAmount.push(ele.identifiedAmount);
          this.applyAmount.push(ele.applyAmount);
          this.applyApprovedAmount.push(ele.applyApprovedAmount);
          this.machineRefuseAmount.push(ele.machineRefuseAmount);
          this.withdrawPassAmount.push(ele.withdrawPassAmount);
        });
        this.updateEchartsOption(this.option);
      } catch (error) {
