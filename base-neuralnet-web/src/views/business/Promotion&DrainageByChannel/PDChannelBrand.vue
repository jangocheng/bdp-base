...
import { getPopularizeBrand } from '@/api/business/PDChannel.js';
  ...
  computed: {
    ...
    option() {
      ...
      this.legendArr.forEach(ele => {
        this.xData.push(ele.brandName);
        seriesData.push(ele.consumptionSum);
      });
      const temBarObj = objClone(barObj);
      const temLineObj = objClone(lineObj);
      if (this.isChain) {
        temLineObj.yAxisIndex = 0;
        temLineObj.data = seriesData;
        seriesArr.push(temLineObj);
      } else {
        temBarObj.data = seriesData;
    ...
    text() {
      const INSTANT_STR = '品牌消费金额排行';
      const CHAIN_STR = '品牌消费金额"环比"';
  ...
  methods: {
    ...
    async getPopularizeBrand() {
      try {
        ...
        const res = await getPopularizeBrand(params);
        if (res.data) {
          this.xData = [];
          this.legendArr = res.data;
          this.updateEchartsOption(this.option);
        }
      } catch (error) {
