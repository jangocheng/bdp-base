...
<template>
  <div class="app-container">
    ...
    <el-card class="main-card table-content" shadow="hover">
      <PDChannelConsumeTable
        :startDate="time_series_2[0]"
        :endDate="time_series_2[1]"
        :channel="channelConsumeChannel"
        ref="PDChannelConsumeTable"
      ></PDChannelConsumeTable>
    </el-card>
    <el-card class="main-card table-content" shadow="hover">
      <PDChannelStroeTable
...
<script>
import { getChanel } from '@/api/business/PDChannel.js';
...
import PDBusChannelApplyEcharts from './PDBusChannelApplyEcharts';
import PDChannelConsumeTable from './PDChannelConsumeTable';
import PDChannelStroeTable from './PDChannelStroeTable';
import PDChannelBrand from './PDChannelBrand';
...
export default {
  data() {
    return {
      ...
      channelEmnu: [],
      channel: '所有', //渠道注册条件
      channelConsumeChannel: '所有', //渠道消费条件
      // 终端枚举
      osTypeEunm,
      osType: '05', //所有终端，不进行过滤
      // channelConsumeOsType: '05', //所有终端，不进行过滤
      pickerOptions: {
        shortcuts: [
          {
            text: '最近一周',
  ...
  methods: {
    ...
    healderQuery() {
      ...
      // 更新渠道报表
      this.$refs.PDChannelTable.getPDAppleTbleData(true);
      //更新“注册到进件”echart数据
      this.$refs.PDBusRegister.getPDBusRegisterData();
      this.$refs.BusApplyChanel.getChannelApplyData();
    },
    /**
     * 渠道消费情况查询按钮
     */
    channelConsumeQuery() {
      this.$refs.PDChannelConsumeTable.getPopularizeConsumptionData(true);
      this.$refs.PDChannelStroeTable.getPopularizeStore();
      this.$refs.PDChannelBrand.getPopularizeBrand();
    },
    // 获取渠道数据
    async getChanel() {
      // 发起渠道请求
    ...
    initTimeSeries() {
      ...
      this.time_series_2[0] = start;
      this.time_series_2[1] = new Date();
    },
  },
};
</script>
