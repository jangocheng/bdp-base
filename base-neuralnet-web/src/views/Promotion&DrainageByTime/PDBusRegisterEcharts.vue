...
import { applyApprovedAmount } from '@/api/business/PDTime';
...
export default {
      ...
      return [
        {
          ...
          data: this.applyPassAmount,
        },
        {
          name: '转化率',
          type: this.isChain ? 'line' : 'line',
          data: this.applyPassRate,
        },
      ];
    },
    sidebarStatus() {
  ...
  methods: {
    ...
    async applyApprovedAmount() {
      const _init = () => {
        this.xData = [];
        this.applyPassAmount = [];
        this.applyPassRate = [];
      };
      try {
        const params = {
        ...
        _init();
        const res = await applyApprovedAmount(params);
        const temResArr = res.data;
        if (!temResArr) return;
        temResArr.forEach(ele => {
          this.xData.push(ele.date);
          this.applyPassAmount.push(ele.applyApprovedAmount);
          this.applyPassRate.push(ele.applyApprovedRate);
        });
      } catch (error) {
