...
class DateJs {
  ...
  /**
   * 获取当前时候前后天数
   * @param {Number} AddDayCount 指定天数 例:往前推1天，即传入-1
   * @param date  当前日期
   * @returns {string}  yyyy-hh-mm
   * @constructor
   */
  ...
  static monthDate(now) {
    ...
    // 分割传入的日期
    const temArr = now.split('-');
    const day = new Date(temArr[0], temArr[1] * 1, 0);
    // 获取第一天
    const lastStart = `${temArr[0]}-${temArr[1]}-01`;
    // 获取最后一天
    const lastEnd = `${temArr[0]}-${temArr[1]}-${day.getDate()}`;
    return [lastStart, lastEnd];
  }
}
