...
import { exportBuyout, getbuyout } from '@/api/duration'
import { IDurationBuyout } from '@/api/types'
...
export default class extends Vue {
    ...
    get exportParam(): IDurationBuyout {
      return {
        ...
        buyoutDateStart: this.buyoutDate[0],
        buyoutDateEnd: this.buyoutDate[1]
      }
    }

    created() {
      this.buyoutDate = [getdatatime(), getdatatime()]
      this.getList()
    }
    ...
    private async getList(btnClick: boolean = true) {
      ...
      let res = await getbuyout(this.ListParam)
      this.tableData = res.data.financeBuyoutCumulativePartnersReportListROList
    ...
    private async exportBuyout() {
      let res = await exportBuyout(this.exportParam)
    }
